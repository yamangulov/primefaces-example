...
@Component("employeeFileDownloadView")
@RequestScoped
@Getter
@Setter
@Log4j2
public class EmployeeFileDownloadView implements Serializable {
    ...
    @Inject
    public EmployeeFileDownloadView(FileService fileService) {
    ...
    public void onload() {
        setStreamedContentListByEmployeeIdAndFileType(id, fileType);
    }

    public void setStreamedContentListByEmployeeIdAndFileType(String id, Integer type) {
        Map<Integer, StreamdContentWrapper> streamedContentMap = new HashMap<>();
        List<File> fileList = fileService.getFilesByEmployeeIdAndType(id, type);
